...
export async function getColorTheme(user_id) {
  const [resultu] = await DB.query(
    "SELECT color_theme FROM users WHERE user_id = ?",
    [user_id]
  );
...
export async function getPatientVisitAnalyticsData() {
  ...
  const latestDate = new Date(
    Math.max(...resultu.map((visit) => new Date(visit.date_of_admission)))
  );
...
export async function getStaffData(search = "") {
  const [result] = await DB.query(
    `SELECT first_name,last_name,profile_img,user_id,role FROM users WHERE role<>'patient' AND role<>'superadmin' AND (first_name LIKE ? OR last_name LIKE ? OR user_id = ?)`,
...
export async function getVisitByID(visit_id) {
  const [visit] = await DB.query(
    `SELECT visit_id,user_id, staff_editable,DATE_FORMAT(date_of_admission,'%Y-%m-%d') AS check_in,DATE_FORMAT(date_of_discharge,'%Y-%m-%d') AS check_out FROM visits WHERE visit_id = ?;`,
    [visit_id]
  );
  return visit[0];
}
...
export async function getPatients(search = "", sort = "descending") {
  ...
  let [result] = await DB.query(sql, [`%${search}%`, `%${search}%`, search]);
  return result;
}

export async function createVisit(patient_id, check_in_date) {
  const isql =
    "SELECT visit_id,DATE_FORMAT(date_of_admission,'%d-%m-%y') as date_of_admission FROM visits WHERE user_id=? AND date_of_discharge IS NULL";
  try {
    ...
    if (iresult.length > 0) {
      return [
        "failed",
        "",
        "Checkout the visit on " +
          iresult[0].date_of_admission +
...
export async function handleDoctorInitialAssessment(formData) {
  ...
  const isql =
    "SELECT record_id FROM doctor_initial_assessment_form WHERE user_id=? AND visit_id=?";
  try {
    const [iresult] = await DB.query(isql, [
      formData.user_id,
      formData.visit_id,
    ]);
    if (iresult.length === 0) {
      ...
      const values = [
        ...
        formData.height,
        formData.pain_assessment,
        formData.weight,
        formData.bmi,
        formData.present_complaints,
        formData.sleep_hours,
        formData.unconscious,
        formData.disoriented,
        formData.bedridden,
        formData.others,
        formData.addictions,
        formData.allergies,
...
export async function getDoctorInitialAssessment(user_id, visit_id) {
  const isql =
    "SELECT * FROM doctor_initial_assessment_form WHERE user_id=? AND visit_id=?";
  try {
    const [iresult] = await DB.query(isql, [user_id, visit_id]);
    if (iresult.length > 0) return ["success", iresult[0], true];
...
export async function addMedicationRecords(payload) {
  ...
  const sql =
    "INSERT INTO medication_orders(visit_id,user_id,date,medicine,route_site,dose,time,anupana,remarks,doctor_id,doctor_name,doctors_sign) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
...
export async function getMedicationRecords(user_id, visit_id) {
  ...
  try {
    ...
    results.forEach((result) => {
      result["doctors_sign"] =
        "data:image/png;base64," + imageToBase64(result["doctors_sign"]);
...
export async function addTreatmentProcedureRecords(payload) {
  ...
  const records = payload.formData;
  const sql =
    "INSERT INTO treat_proc_ord(visit_id,user_id,date,time,treatment_procedure,medicine,site_loc,no_of_days,precautions,doctor_id,doctor_name,doctors_sign) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
  for (const record of records) {
    try {
      const [result] = await DB.query(sql, [
        visit_id,
...
export async function addVitalChartRecords(payload) {
  ...
  const records = payload.formData;
  const sql =
    "INSERT INTO vital_chart_form(visit_id,user_id,date,time,temperature,pulse,bp,weight,remarks,doctor_id,doctor_name,doctors_sign) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
  for (const record of records) {
    try {
      const [result] = await DB.query(sql, [
        visit_id,
...
export async function handleDischargeForm(formData) {
  const isql =
    "SELECT discharge_id FROM discharge_summary WHERE user_id=? AND visit_id=?";
  try {
    const [iresult] = await DB.query(isql, [
      formData.user_id,
      formData.visit_id,
    ...
    if (iresult.length === 0) {
      ...
      const values = [
        ...
        formData.restricted_activities,
        formData.advices,
...
export async function addNursingCarePlanRecords(payload) {
  ...
  const records = payload.formData;
  const sql =
    "INSERT INTO nursing_care_plan(visit_id,user_id,date,time,problem_identified,suggestions,nurse_id,nurse_name,nurses_sign) VALUES(?,?,?,?,?,?,?,?,?)";
  for (const record of records) {
    try {
      const [result] = await DB.query(sql, [
        visit_id,
...
export async function getNursingCarePlanRecords(user_id, visit_id) {
  ...
  try {
    ...
    results.forEach((result) => {
      result["nurses_sign"] =
        "data:image/png;base64," + imageToBase64(result["nurses_sign"]);
      actual_result.push(result);
    });
    return ["success", actual_result, "Successfully Fetched Records"];
  } catch (err) {
...
export async function addMedicationAdministrationRecords(payload) {
  ...
  const records = payload.formData;
  const sql =
    "INSERT INTO medication_administration_records(visit_id,user_id,date,time,medication_name,dose,frequency,anupanam,nurse_id,nurse_name,nurses_sign) VALUES(?,?,?,?,?,?,?,?,?,?,?)";
  for (const record of records) {
    try {
      const [result] = await DB.query(sql, [
        visit_id,
  ...
  try {
    ...
    try {
      ...
      return [
        "success",
        "",
        toTitleCase(actual_role) + " Updated Successfully",
      ];
    } catch (err) {
      console.log(err);
