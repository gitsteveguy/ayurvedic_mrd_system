...
const MedicationAdministrationForm = (props) => {
    const current_patient_id = getCurrentPatientID();
    const current_patient_visit_id = getCurrentPatientVisitID();
    const post_api_url = 'http://localhost:5000/api/add_medication_admistration_records'
    ...
    return (
        <>
            <h1>Medication Administration Records</h1>
            <form name='medicationadministration' id='medicationadministration' inert={inert} onSubmit={handleSubmit}>
                <RepeatingComponent subtitle='Text'>
                    <IRow>
                        ...
                        <ICol>
                        <ITxtInput name='medication_name[]' label='Medication Name' />
                        </ICol>
                        </IRow>
                        <IRow>
                        <ICol>
                        <ITxtInput name='dose[]' label='Dose' />
                        </ICol>
                        <ICol>
                        <ITxtInput name='frequency[]' label='Frequency' />
                        </ICol>
                        <ICol>
                        <ITxtInput name='anupanam[]' label='Anupanam' />
                        </ICol>
                    </IRow>
                </RepeatingComponent>
              {props.inert==='false' && <IRow>
                    ...
                    <Link to='/patients/view_patient_visit'><button type='button' className='danger-btn formbtn' tooltip='Cancel'>Cancel</button></Link> 
                        <button type='submit' form="medicationadministration" className=' primary-btn formbtn' tooltip='Submit' value="submit">Submit</button>
                    </ICol>
                </IRow>}
            </form>
        </>
